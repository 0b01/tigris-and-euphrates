...
impl<E: Evaluator> Negamax<E> {
        ...
        <<E as Evaluator>::G as Game>::M: Copy,
    {
        ...
        for m in moves.iter() {
            ...
            // Check if the same player continues after this move
            // If so, don't negate the evaluation (same player's perspective)
            let value = if E::G::same_player_continues(&new) {
                self.negamax(&mut new, depth - 1, alpha, beta)
            } else {
                -self.negamax(&mut new, depth - 1, -beta, -alpha)
            };
            best = max(best, value);
    ...
    <E::G as Game>::M: Copy,
{
    fn choose_move(&mut self, s: &<E::G as Game>::S) -> Option<<E::G as Game>::M> {
        ...
        for &m in moves.iter() {
            ...
            // Check if the same player continues after this move
            let value = if E::G::same_player_continues(&new) {
                self.negamax(&mut new, self.max_depth - 1, best, BEST_EVAL)
            } else {
                -self.negamax(&mut new, self.max_depth - 1, WORST_EVAL, -best)
            };
