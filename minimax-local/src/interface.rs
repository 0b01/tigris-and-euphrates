...
pub trait Game: Sized {
    ...
    /// Returns true if the same player moves again after this state.
    /// When true, negamax will NOT negate the evaluation, treating consecutive
    /// moves by the same player as a single "super-turn".
    /// Default is false (players alternate after each move).
